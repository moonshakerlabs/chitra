...
import { Folder, FolderPlus, AlertTriangle, Check } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Checkbox } from '@/components/ui/checkbox';
import { Card } from '@/components/ui/card';
import { createChitraFolder, validateChitraFolder, setStorageFolderPath } from '@/core/storage/folderService';
...
const FolderSelect = ({ onComplete, onBack }: FolderSelectProps) => {
  ...
  const handleCreateNewFolder = async () => {
    ...
    setLoading(true);
    const result = await createChitraFolder();
    setLoading(false);

    if (result.success) {
      toast({
        title: 'CHITRA Folder Created',
        description: 'Your data will be stored in the CHITRA folder.',
      });
      onComplete();
    } else {
      toast({
  ...
  const handleUseExistingFolder = async () => {
    ...
    // For now, check if CHITRA folder exists in Documents
    const isValid = await validateChitraFolder('Documents/CHITRA');
    
    if (isValid) {
      await setStorageFolderPath('Documents/CHITRA');
      toast({
        title: 'CHITRA Folder Found',
        description: 'Using existing CHITRA folder for data storage.',
      });
      onComplete();
    } else {
      toast({
      ...
      <div className="flex-1">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">
            Choose where CHITRA should store your data on this device
          </p>
        </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Start fresh with a new data folder
                </p>
              </div>
              {selectedOption === 'new' && (
